...
export async function POST(request: Request) {
    ...
    `;
        ...
        const aiContent = response.choices[0].message.content!.trim().replaceAll("```", "");
        ...
        return NextResponse.json({ error: "Failed to generate meal plan. Please try again later." }, { status: 500 });
    }
}
